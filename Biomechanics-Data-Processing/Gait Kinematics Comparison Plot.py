...
HEALTHY_GROUP_DATA_FILES = [
    {
        'file_path': r"D:\uni\post stroke\0core\SUBJ75(healthy female)\results\IK75.mot",
        'initial_time': 2.97,  # <--- Manual initial time for this healthy subject
        'final_time': 4.06,  # <--- Manual final time for this healthy subject
        'gait_events_file': r"D:\uni\post stroke\0core\SUBJ75(healthy female)\grf\SUBJ75 (1).csv"
...
INDIVIDUAL_SUBJECT_DATA_FILES = {
    'severe Case': {
        'file_path': r"D:\uni\post stroke\0core\Severe\results\5IK.mot",
        'initial_time': 2.95999999999999996447,  # <--- CHANGED TO NONE: Will now attempt to use gait_events_file or full trial
        'final_time': 4.46999999999999975131,    # <--- CHANGED TO NONE
        'gait_events_file': r"D:\uni\post stroke\0core\Severe\grf\BWA5.csv"
...
# NEW: Define GAIT_CYCLE_EVENTS dictionary for colors
GAIT_CYCLE_EVENTS = {
    'Initial Contact': 'green',
    'Toe Off': 'red',
    'Opposite Foot Strike': 'blue',
    'Left_Foot Strike': 'green', # Example from your previous event data
    'Right_Foot Off': 'blue',
    'Right_Foot Strike': 'red',
    'Left_Foot Off': 'orange',
}
...
def normalize_gait_cycle(df, initial_time, final_time):
    ...
    df_cycle = df_norm[(df_norm['time'] >= initial_time) & (df_norm['time'] <= final_time)].copy() # .copy() here to avoid warning
...
def plot_gait_kinematics_comparison():
    ...
    if HEALTHY_GROUP_DATA_FILES:
        ...
        handles.append(plt.fill_between([], [], [], color='lightgray', alpha=0.5)) # Corrected function name
